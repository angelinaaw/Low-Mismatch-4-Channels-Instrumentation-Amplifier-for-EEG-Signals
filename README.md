# Low-Mismatch-4-Channels-Instrumentation-Amplifier-for-EEG-Signals
...
| Power Consumption per Channel | <200uW                | At 3.3 V supply (might be lower if use lower supply voltage                                             |
...
| Low Pass Filter (LPF)      | Suppress high-frequency noise from the bipotential signal                     | Moderate                  | Simple analog RC circuit using several capacitors. Simulations are challenging to execute for switching capacitors.                                   |
| Frequency Divider          | Divide clock signal based on required frequencies                            | Moderate    | - Converts high-freq clock into clkA, clkB, clkC using sequential logic <br> - Uses flip-flops and handles clock edges <br> - Easily scalable.  |
| Clock Inverter            | Generate CLK and CLK inverted throught delay with specific frequency       | Low                  | Used for LPF and CS. Only consist of Transmission gates and inverters.                                                                 |
